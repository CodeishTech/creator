...
export default function AllSubjects(){
  ...
  function DeleteSubject(id){
    axios.get(serverUrl+'delete_subject/'+id, {
      headers
    })
    .then(result => {
  ...
  return (
    <div align="center" justify="center">
      <div className="space-y-4 shadow-2xl dark:shadow-gray-400 p-4">
        <i><p className="text-gray-900 dark:text-orange-500 text-2xl"><u>All Subjects</u></p></i> <br/>
              ...
              {data && data.map((item, index) => {
                return (
                  <tr className="bg-white border dark:bg-gray-900 dark:border-gray-200">
                    ...
                    <td className="px-6 py-4">
                      <Link to={"editsubject/"+item._id}><button className="bg-blue-600 hover:bg-white hover:border-blue-600 hover:text-blue-600 rounded-md p-1 font-medium text-white hover:underline"><FontAwesomeIcon icon={faPenToSquare} /> Edit</button></Link>
                    </td>
                    <td className="px-6 py-4">
                      <button onClick={()=>DeleteSubject(item._id)} className="bg-red-600 hover:bg-white hover:border-red-600 hover:text-red-600 rounded-md p-1 font-medium text-white hover:underline"><FontAwesomeIcon icon={faTrashAlt} /> Delete</button>
                    </td>
                  </tr>
                )
              })}
